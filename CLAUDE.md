...
### Salón de la Fama (`/fama`) — escenario 3D de las Leyendas
`/fama` is a lineup of 15 community legends (two rows, back row on a step) plus two flying Daevas flanking the group, rendered as 2.5D game characters (three.js). Data + anchors in `src/lib/fama/leyendas.ts` (name, guild, `efecto`, per-image anchors in 0..1 image coords; `vuela: {x,y,z}` takes a character out of the row and makes it hover — keep flyers low/at the sides, placing them above the group forces the camera back and shrinks everyone; list order = row order; `HUECO` leaves a central gap where the leader (`lider: true`, permanent halo) hovers; sub-leaders sit in the slots next to the gap); engine in `src/lib/fama/escenario.ts` (+ `shaders.ts`, `efectos.ts` with the 16 hover gestures, `particulas.ts`); component `src/components/fama/LegendsStage.tsx`; CSS `.ly-*` in globals.css. The backdrop painting `public/fama/fondo.webp` comes from `scripts/fama/generar_fondo.py` (its lower `PAISAJE_BASE` fraction, the painted lake, is hidden under the reflective floor). Character art lives in `public/fama/leyendas/<id>.webp` + `<id>-relieve.png` and is produced by `scripts/fama/generar_leyendas.py` (AI Horde, anonymous, free) and `scripts/fama/procesar_leyendas.py` (rembg cutout + relief map + anchor grids in `scripts/fama/raw/rejilla/`). To add a legend: prompt → generate → process → read anchors on the grid → add the entry. Verify with headless Chromium on the real GPU (`--use-angle=d3d11 --enable-gpu --ignore-gpu-blocklist`); SwiftShader hangs on hover screenshots. The old Titanes hall (`TitanesHall.tsx`, `titanes.ts`, `/admin/titanes`) is no longer shown on the site.
